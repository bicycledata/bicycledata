...
def post_process():
    ...
    try:
        ...
        log_file = os.path.join(
            log_path, datetime.now().strftime("%Y%m%d-%H%M%S") + ".log"
        )
        createDirIfNeeded(log_path)
        log(
            log_file,
            datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " | postprocess start",
        )
        ...
        for device in os.listdir(devices_dir):
            ...
            for session in os.listdir(sessions_dir):
                ...
                bicyclebutton_file = os.path.join(
                    sessions_dir, session, "bicyclebutton"
                )
                ...
                if os.path.isdir(postprocess_dir):
                    ...
                    log(
                        ...
                        + log_session_action
                        + " | Execution time[s]: "
                        + str(time.time() - log_session_start),
                    )
                    continue
                if not (
                    file_sanity_check(bicyclebutton_file)
                    and file_sanity_check(bicyclegps_file)
                    and file_sanity_check(bicyclelidar_file)
                ):
                    log_sessions_skipped_ctr += 1
                ...
                with open(
                    os.path.join(
                        sessions_dir, session, "postprocess", "postprocess.txt"
                    ),
                    "w",
                ) as f:
    ...
    log(
        ...
        str(
            datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            + " | postprocess end results"
            + " | sessions discovered: "
            + str(log_sessions_discovered_ctr)
            + " | sessions processed: "
            + str(log_sessions_processed_ctr)
            + " | sessions skipped: "
            + str(log_sessions_skipped_ctr)
        ),
    )
