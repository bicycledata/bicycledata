...
def signup():
    ...
    if len(users) > 100:
        flash(
            "There is unusually high traffic at the moment. Please try to signup later again."
        )
...
def user_sessions():
    ...
    for s in flask_login.current_user.sessions:
        ...
        gps_path = os.path.join(
            "data", "v2", "devices", device, "sessions", date, "bicyclegps"
        )
        info_path = os.path.join(
            "data", "v2", "devices", device, "sessions", date, "session.info"
        )

        # Check if session should be included based on GPS data
        if not all_sessions and not (
            os.path.exists(gps_path) and os.path.getsize(gps_path) > 50
        ):
            continue
