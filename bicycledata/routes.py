        ...
        data["sensors"] = [
            {
                ...
                "git_url": "https://github.com/bicycledata/sensor_template.git",
                "git_branch": "main",
                "entry_point": "sensor:main",
                "restart": False,
                "args": {"upload_interval": 5},
            }
        ]
...
def v2_devices_ident_session(ident, session):
    try:
        ...
        sensors = [
            ...
            if os.path.isfile(os.path.join(session_dir, name)) and "." not in name
        ]
        sensors.sort()
