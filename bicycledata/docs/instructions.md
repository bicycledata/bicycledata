...
<table>
  ...
  <tr>
    <td align="center">
      <img src="/static/images/devices/KatjaKircherVTI_TrafficLogger-033.jpg" style="width: 250px; max-width: 100%;" alt="Push button attachment point"><br>
      <strong>USB-contact for push button</strong><br>
      Attach the button to the logger via USB-C.
    </td>
    <td align="center">
      <img src="/static/images/devices/ComCy_BoxInside.jpg" style="width: 250px; max-width: 100%;" alt="Box with USB-C-cable in charging port"><br>
      <strong>Charge the box</strong><br>
...
<table>
  <tr>
    <td align="left">
      ...
      Also the password should be set to `bicycledata` 
      Use 2.4 GHz.
    </td>
    <td align="center">
...
1. Activate your hotspot (`bicycledata`).
2. Start the Garmin Varia (press the big button on the side for 2 seconds) – the LED on the side will turn green, then red. [Can be different for different devices.]
3. Start the logger with the ON/OFF-switch. It takes a minute or two for the logger to start. You can see the battery status of the logger on the display. The display turns off after a while.
  a. **WiFi**: The WF-led indicates that the logger has connected to your hotspot and can download updates or new configurations.
  b. **WiFi**: When the WF-led turns off, you can shut off your hotspot if you want.
  c. **Radar**: The RA-led indicates that the logger has connected to the Garmin Varia.
  d. **GPS**: The GP-led indicates that the logger has got a GPS fix. This can take a while, 4-7 minutes is not unusual.

## Data upload and shutting down
1. Activate your hotspot again (`bicycledata`).
2. Press the data button for 5 seconds (until the LEDs start flashing).
  a. All three LEDS are flashing (data is being uploaded).
  b. Wait until all LEDs have turned off.
3. Turn off the Garmin Varia by pressing the button for 2 seconds. All LEDs on the Garmin should be off.
4. Turn off the logger with the ON/OFF/Charge-switch. You can see the current charging level on the display for a few seconds. If you do not set the switch to OFF, the battery will drain.
...
You see the charging level on the display when you attach and detach the cable. 
...
You go to the homepage and log in here: https://bicycledata.vti.se/ 

## After each ride

OBS: This will be updated with additional functionality

Log in on your dashboard after each ride to:
• mark whether the data originate from a real ride or if you only tested the logger and ”played” with it (in this case click “hide”)
• mark whether you rode on your own (write “0”) or in the company of others (indicate how many people you rode with)
• (optional: note the battery status of the logger in the beginning and in the end of the ride—this is for future development of the logger)
• comment certain events or other things we should know about the ride (note anything you find relevant, regardless of whether it is about overtaking events, traffic situations, the weather, comments about the logger and handling; we will develop the dashboard further over time, let us know what you would like to see). 
