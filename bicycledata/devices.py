...
def read_v2_sessions(ident, all=False, show_hidden=False):
    ...
    sessions = [
        {
            "device": ident,
            "date": name,
            "hidden": SessionInfo.is_hidden(
                os.path.join(directory, name, "session.info")
            ),
        }
    ...
    if not all:
        sessions = [
            s
            for s in sessions
            if os.path.exists(os.path.join(directory, s["date"], "bicyclegps"))
            and os.path.getsize(os.path.join(directory, s["date"], "bicyclegps")) > 50
        ]
...
def write_v2_config_file(ident, config):
    ...
    with open(
        os.path.join("data", "v2", "devices", ident, "bicycleinit.json"), "w"
    ) as file:
        json.dump(config, file, indent=2)
        ...
        with open(file_path, "rb") as f:
            ...
            while file_size > 0:
                ...
                if b"\n" in buffer:
                    ...
                    last_ping_time = datetime.strptime(
